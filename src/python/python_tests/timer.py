...
CSV_FILE = "/home/kali/Téléchargements/Morse-Sequence/graphs/timing_results_python.csv"
...
def timer_comparison():
    ...
    for k in list_faces:
        ...
        # --- Bloc : Création SimplexTree + MorseSequence ---
        start_init = time.time()
        st = SimplexTree(MakeFacesVectorized1(k, k))
        ms = MorseSequence(st)
        end_init = time.time()
        print(f"Initialisation (SimplexTree + MorseSequence) : {end_init - start_init:.6f} secondes\n")
        ...
        print(f"Préparation Max python pur : {end_max_pre - start_max_pre:.6f} secondes")
        ...
        print(f"Packing Max pour C++ : {end_pack_max - start_pack_max:.6f} secondes")
        ...
        print(f"Préparation Min python pur : {end_min_pre - start_min_pre:.6f} secondes")
        ...
        print(f"Packing Min pour C++ : {end_pack_min - start_pack_min:.6f} secondes")
    ...
    total_end = time.time()
    print(f"\n⏱️ Temps total timer_comparison() : {total_end - total_start:.2f} secondes")
