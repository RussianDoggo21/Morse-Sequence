 ...
 */
RefMap::RefMap(MorseSequence& ms, m_sequence W) : Utils(ms, W), W(W) {
    for (const auto& item : this->W) {
        ...
        if (std::holds_alternative<node_ptr>(item)) {
            ...
            element.pair = {{sigma_ptr, b_sigma}, {tau_ptr, bitmap(dim_crit)}};
        }
        else {
...
/*
tsl::robin_map<node_ptr, bitmap> RefMap::get_full_bitarray() {
...
void RefMap::print_full_bitarray(const m_sequence& W) {
